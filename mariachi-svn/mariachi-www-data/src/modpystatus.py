...
modpystatus is a quick and dirty way to figure out what your script is doing, 
like PHP's phpinfo() page. just import modpystatus and add it to your page's 
output variables, like this:
...
class modPyStats:
  ...
  def statusRow(self,name,value,desc):
    return """<tr>
    <td bgcolor=#ffffcc width=120 align=left valign=top><i><font size=3>"""+name+"""</font></i></td>
    <td bgcolor=#fefefe width=25 align=left valign=top><font size=3><pre>= """+value+"""</pre></font></td>
    </tr>"""
  ...
  def status(self,req):
    ...
    self.page=self.page+self.statusRow('FieldStorage',self.statusPp2str(y.list),
                                       'Field Storage: pass any fields that were included with the page')

    for i in self.stats:

      if i[0]=='mp_table': value=self.statusPp2str(self.statusMp2pp(getattr(req,i[1])))
      if i[0]=='mp_server': value=self.statusPp2str(self.statusMp2pp(getattr(req,i[1])))
      if i[0]=='connection': value=self.statusPp2str(getattr(req.connection,i[1]))
      if i[0]=='string': value=self.statusPp2str(getattr(req,i[1]))

      self.page=self.page+self.statusRow(i[1],value,i[2])

    for i in self.apstats:
      if i[0]=='string': value=self.statusPp2str(getattr(apache,i[1]))
      self.page=self.page+self.statusRow(i[1],value,i[2])

    self.page=self.page+self.statusRow('sys.path',self.statusPp2str(sys.path),'sys.path')
    self.page=self.page+self.statusRow('os.environ',self.statusPp2str(os.environ),'Environment Variables')
