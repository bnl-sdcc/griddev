...
#
#  Vihuela Data Acquisition Library
...
log.debug('mariachi.daq: Found python %d.%d' % (major, minor) )  
if major ==  2 and minor >= 4:
    import myurllib224 as urllib2
else:
    log.fatal('vihuela.daq: This module requires Python 2.4')
    sys.exit(0)



class GridsiteUploader(threading.Thread):
    """
    ...
    def __init__(self, config,section,cgiscript="gridsite-admin.cgi"):
        ...
        self.klassname="vihuela.daq.GridsiteUploader"
        self.certfile = config.get('daemon','cert_file')
        self.keyfile = config.get('daemon','key_file')
        self.strict= config.get('daemon','strict')
        self.upload_host = config.get('daemon','upload_host')
        ...
        self.interval = float(config.get(section,'upload_interval'))
    ...
    def run_upload(self):
       ...
       """
        self.log.debug("vihuela.daq.GridsiteUploader.run_upload(): Beginning run...")
        ...
        try:
            ...
            self.log.error('vihuela.daq.GridsiteUploader.run_upload(): Problem connecting to remote URL. Check network, proxies, config.')
        ...
        if remotestat:
            ...
            # debug assessment
            self.log.debug('vihuela.daq.gridsiteUploader.run_upload(): Got remote set: %s ' % remotefiles)
            self.log.debug('vihuela.daq.gridsiteUploader.run_upload(): Got local set: %s ' % localfiles)
            ...
            for i in sendset:
                sendlist.append(i)
            sendlist.sort()
            self.log.debug('vihuela.daq.gridsiteUploader.run_upload(): Got send list: %s ' % sendlist)
            if len(sendlist) == 0:
                self.log.info('vihuela.daq.gridsiteUploader.run_upload(): [%s] No files to send.' % self.section )
            try:
                for f in sendlist:
                    if f.type == 'dir':
                ...
                self.log.debug('vihuela.daq.gridsiteUploader.run_upload(): Problem uploading file. Check remote URL config, permissions.')
    ...
    def _uploadFile(self, filename):
        ...
        """
        self.log.debug('vihuela.daq.gridsiteUploader._uploadFile(): Setting x509...')
        ...
        self.log.debug('vihuela.daq.gridsiteUploader._uploadFile(): Uploading %s' % fullpath)
 
        # filename e.g. "/subdir/newsubdir"
        self.log.debug('vihuela.daq.gridsiteUploader._uploadFile(): Uploading %s' % filename)
        (parents,newfile) = os.path.split(filename)
        self.log.debug('vihuela.daq.gridsiteUploader._uploadFile(): parents = %s tail=%s' % (parents,newfile))
        
        # build CGI URL
        url = "https://%s%s" % (self.upload_host, self.base_path)
                
        self.log.debug('vihuela.daq.gridsiteUploader._uploadFile(): URL step 1: %s' % url)
        if parents and len(parents) > 1:
            url = "%s%s/" % ( url, parents)
        else:
            url = "%s/" % url
        self.log.debug('vihuela.daq.gridsiteUploader._uploadFile(): URL step 2: %s' % url)
        url = "%s%s" % (url, self.cgi_exec)
        self.log.debug('vihuela.daq.gridsiteUploader._uploadFile(): URL step 3: %s' % url)
        
        self.log.debug('vihuela.daq.gridsiteUploader._uploadFile():  CGI upload URL:  %s'  % url )
        ...
        self.log.debug('vihuela.daq.gridsiteUploader._uploadFile():  params:   %s'  %  params )
    ...
    def _mkDir(self, newname):
        ...
        """
        self.log.debug('vihuela.daq.gridsiteUploader._mkDir(): Setting x509.')
        urllib2.HTTPSHandler.certfile = self.certfile
        urllib2.HTTPSHandler.keyfile = self.keyfile
        urllib2.HTTPSHandler.strict = self.strict
        ...
        self.log.debug('vihuela.daq.gridsiteUploader._mkDir(): Creating %s' % newname)
        (parents,newdir) = os.path.split(newname)
        self.log.debug('vihuela.daq.gridsiteUploader._mkDir(): parents = %s tail=%s' % (parents,newdir))
        
        # build CGI URL
        url = "https://%s%s" % (self.upload_host, self.base_path)
        self.log.debug('vihuela.daq.gridsiteUploader._mkDir(): URL step 1: %s' % url)
        if parents and len(parents) > 1:
            url = "%s%s/" % ( url, parents)
        else:
            url = "%s/" % url
        self.log.debug('vihuela.daq.gridsiteUploader._mkDir(): URL step 2: %s' % url)
        url = "%s%s" % (url, self.cgi_exec)
        self.log.debug('vihuela.daq.gridsiteUploader._mkDir(): URL step 3: %s' % url)

        # debug info
        self.log.debug('vihuela.daq.gridsiteUploader._mkDir():  CGI upload URL:  %s'  % url )
        params = { 'cmd' : 'edit' ,  'button' : 'Create', 'file' : newdir }
        self.log.debug('vihuela.daq.gridsiteUploader._mkDir():  params:   %s'  %  params )
    ...
    def _getRemoteFileSet(self, dirpath):
        ...
        """
        self.log.debug('vihuela.daq.gridsiteUploader._getRemoteFileSet():...')
        rfs = sets.Set()
        listing = self._listDir(dirpath)
        #self.log.debug('vihuela.daq.gridsiteUploader._getRemoteFileSet(): Response %s' % listing )
        ...
        for f in files:
            ...
            self.log.debug('vihuela.daq.gridsiteUploader._getRemoteFileSet(): File: %s' % f )
            rfs.add(ulf)
        for d in dirs:
            dname = "%s%s" % (dirpath, d[0])
            uld = UploadFile( dname, int(d[1]), file_type="dir")
            rfs.add(uld)
            self.log.debug('vihuela.daq.gridsiteUploader._getRemoteFileSet(): Dir: %s' % d )
            rfs.union_update(self._getRemoteFileSet("%s%s/" % (dirpath, d[0])))
        
        #self.log.debug('vihuela.daq.gridsiteUploader._getRemoteFileSet(): Got set: %s' % rfs)
        return rfs
    ...
    def _listDir(self,dirpath):
        ...
        '''
        self.log.debug('vihuela.daq.gridsiteUploader._listDir(): Listing remote dir path: %s' % dirpath )
  
        fullpath="%s%s" % ( self.base_path , dirpath )
        self.log.debug('vihuela.daq.gridsiteUploader._listDir(): Listing path:%s' % fullpath)
        ...
        log.debug('vihuela.daq.gridsiteUploader._listDir(): returning response')
    ...
    def _getLocalFileSet(self):
        ...
        """
        self.log.debug('vihuela.daq.gridsiteUploader._getLocalFileSet(): ...')
        lfs = sets.Set()
        self.log.debug('vihuela.daq.gridsiteUploader._getLocalFileSet(): local_base is %s' % self.local_base)
        for root, dirs, files in os.walk(self.local_base):
            ...
            for f in files:
                self.log.debug('vihuela.daq.gridsiteUploader._getLocalFileSet(): checking file %s' % f )
                ff = os.path.join(root,f)
                si = os.stat(ff)
                rf = self._strip_localbase(ff)
                uf = UploadFile(rf, si.st_size, file_type="file" )
                lfs.add(uf)
        #self.log.debug('vihuela.daq.gridsiteUploader._getLocalFileSet(): Got local set %s' % lfs)
        return lfs
...
class UploadManager(object):
    """
    vihuela.daq.UploadManager
    
    This class manages all the Uploader object threads. 
...
#        threading.Thread.__init__(self)
        self.log = logging.getLogger()
        self.log.debug('vihuela.daq.UploadManager.__init__()...')
        self.config = config
...
#    def run(self):
    def mainloop(self):
        self.log.debug('vihuela.daq.UploadManager.run(): creating threads...')
        ...
        try:
            while(1):
                time.sleep(3)
                self.log.debug('vihuela.daq.UploadManager.mainloop(): Checking for interrupt...')
        ...
        self.log.debug('vihuela.daq.UploadManager.run(): all threads joined.')    
        self.log.debug('vihuela.daq.UploadManager.run(): Done.')
...
def urllibx509httpsGet(_cert_file,_key_file,url):
    ...
    log.debug('vihuela.daq.urllibx509httpsGet(): making opener')
    opener= urllib.URLopener(key_file = _key_file, cert_file= _cert_file )
    log.debug('vihuela.daq.urllibx509httpsGet(): opener keyfile is %s'% opener.key_file)
    log.debug('vihuela.daq.urllibx509httpsGet(): opener certfile is %s' % opener.cert_file )
    log.debug('vihuela.daq.urllibx509httpsGet(): opening url...')
    response = opener.open(url, data=None)
    log.debug('vihuela.daq.urllibx509httpsGet(): returning response')
    return response.read()
