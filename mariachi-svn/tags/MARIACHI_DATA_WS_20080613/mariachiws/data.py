...
logobject.debug("DataHandler.generateCountsGraph(): About to import rpy...")        
logobject.debug("DataHandler.generateCountsGraph(): Finished importing rpy...")
...
class SiteFileHandler(object):
    ...
    def getData(self, type, start, end):
        ...
        for item in datalist:
            if item.datetime > start and item.datetime < end:
                ...
                #logobject.debug("data.getData(): Item in requested interval. Keeping...")
            else:
                pass
                #logobject.debug("data.getData(): Item outside requested interval. Skipping...")
        logobject.debug("data.getData(): Got data list of %d items, filtered to %d." % (len(datalist),len(returnlist) ))
        return returnlist
    ...
    def _checkDate(self, basename, start, end):
        '''
            Checks to see if date for a single file in the file basename is between 
            start and end. 
            Returns boolean
        ...
        if len(datestr) == 8:
            ...
            m = int(datestr[4:6])
            d = int(datestr[6:])
            logobject.debug("SiteFileHandler._checkDate(): Making file datetime %d %d %d " % (y,m,d))
            ...
            if dt_late >= start and dt_early <= end:
                logobject.debug("SiteFileHandler._checkDate(): File date is in interval.")
                ret = True
        else:
            logobject.debug("SiteFileHandler._checkDate(): File date is not in interval.")
        return ret
  
  
   # def _getAll(self):
   #     '''
   #     Lists all data files (that match appropriate globs) in sd 
   #     subdir of base directory.
   #            
   #     '''
   #     datadir="%s%s" % (self.rootdir, SDSUBDIR) 
   #     logobject.debug("SiteFileHandler.getAll(): Listing %s" % datadir)
   #     files = os.listdir(datadir)
   #     filestoread=[]
   #     for fn in files:
   #         (head, bfn) = os.path.split(fn)
   #         logobject.debug("file base name is %s" % bfn)
   #         if self._matchFiles(bfn):
   #             filestoread.append("%s/%s" % (datadir,fn))
   #     return filestoread 
        ...
        for fn in filelist:
            ...
            for line in lines:
                ...
                if t == "gts":
                    ...
                    logobject.warn("SiteFileHandler._parseFiles(): Couldn't figure out what to do with line %s in file %s" % (line, fn))
        return datalist
...
class DataHandler(object):
    ...
    def generateCountsGraph(self, counts, sitename, widthpx=648, resol=72,  ):
        ...
        counts_data = {"datetime":[],
                     "c1":[]}
        tmpgraphfile= tempfile.NamedTemporaryFile()
        logobject.debug("DataHandler.generateCountsGraph(): Temp graph filename = %s" % tmpgraphfile.name)
        ...
        if len(cts) > 0:
            ...
            robj.dev_off()
            import imghdr
            imgtype = imghdr.what(tmpgraphfile.name)
            logobject.debug("DataHandler.generateCountsGraph(): OK: What is our tempfile? = %s" % imgtype )
            f = open(tmpgraphfile.name)
        else:
            logobject.debug("DataHandler.generateCountsGraph(): No data. Generating proper error image...")
            logobject.debug("DataHandler.generateCountsGraph(): Temp error image filename = %s" % tmpgraphfile.name)
            
            import Image
            import imghdr
            imf = Image.open(self.config.get('data','nodatapng'))
            imf2 = imf.resize( (imgwidth, imgheight), Image.NEAREST)
            imf2.save(tmpgraphfile.name )
