...
// Status information
  ...
  if (file_exists("../" . $site . "_last_status")) {
    ...
    echo "<ul><li>No last status information available</li?></ul>";
  }

  if (file_exists("../" . $site . "_status_msg")) {
    $last_time = filemtime("../" . $site . "_status_msg");
    if (time() > ($last_time + 900)) {
      $status = "Offline";
      $status_date = date("H:i T, j F Y", $last_time + 300);  
    } else {
      $handle = fopen("../" . $site. "_status_msg", "r", 1024);
      if ($handle) {
        $status = trim(fgets($handle));
        $status = ucfirst($status);
        fclose($handle);
    ...
    $last_status_date = date("H:i T, j F Y", filemtime("../" . $site . "_status_msg"));  
